...
def show_price_graph(search_results, price_column="Unit Price (USD)", product_column="Product Name", unit_column="Unit"):
    ...
    chart = alt.Chart(df_plot).mark_bar(size=40).encode(
        x=alt.X("File:N", title="Source File", axis=alt.Axis(labelAngle=0)),
        y=alt.Y("Price (USD):Q", title="Price (USD)"),
        tooltip=[
            alt.Tooltip("Product", title="Product"),
            alt.Tooltip("Price (USD)", title="Price"),
            alt.Tooltip("Unit", title="Unit"),
            alt.Tooltip("File", title="File")
        ],
        color=alt.Color("File:N", legend=None)
    ).properties(
        width=600,
        height=350,
        title="💲 Price Comparison for Your Search"
    )
